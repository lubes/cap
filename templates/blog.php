<section class="articles-wrap">
    <div class="container">
        ...
        <ul class="post-blocks">
            ...
            <li <?php post_class('featured-block post-block category-all'); ?> style="background:url(<?php if(has_post_thumbnail) { echo the_post_thumbnail_url(); }?>) no-repeat center;background-size: cover;">
        ...
        <!-- Category Page -->
        <?php $categories = get_the_category();$category_id = $categories[1]->cat_ID;?>
        <?php if(is_category() && !is_paged()):?>
        <ul class="post-blocks">
            <?php $args = array( 'posts_per_page' => 8, 'post_type'=> 'post', 'category__and' => array( 12 ) );
            $myposts = get_posts( $args );  $i = 0;
            foreach ( $myposts as $post ) : setup_postdata( $post );$i++; ?>
                <li <?php post_class('featured-block post-block category-all'); ?> style="background:url(<?php if(has_post_thumbnail) { echo the_post_thumbnail_url(); }?>) no-repeat center;background-size: cover;">
                    <a class="post-content <?php if(!has_post_thumbnail()) { echo the_field('color'); }?>" href="<?php echo the_permalink();?>">
        ...
        <section class="all-posts">
            <div class="container">
                <div class="row grid">
                    ...
                    <?php } else if(is_category() && !is_paged()) { ?>
                    
                        <?php $cat_id = $wp_query->get_queried_object_id(); ?>
                        <?php $my_query = new WP_Query( 'cat=-12,'.$cat_id.'&orderby=date&order=DESC' ); ?>
                        <?php while ( $my_query->have_posts() ) : $my_query->the_post(); ?>
                            <?php get_template_part('templates/content', get_post_type() != 'post' ? get_post_type() : get_post_format()); ?>
                        <?php endwhile; ?>                    
